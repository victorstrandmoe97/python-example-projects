...
echo "<<<<<<<<<<<<Starting Node.js Software Composition Analysis>>>>>>>>>>>>"

echo "<<<<<<<<<<<<Preparing directories>>>>>>>>>>>>"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BASE_DIR="$SCRIPT_DIR"
PROJECT_DIR="$SCRIPT_DIR/nodejs-example"
...
npm ls --all --json > "$FULL_DEPS_TREE_PATH"
echo "<<<<<<<<<<<<Created dependency tree>>>>>>>>>>>>"

echo "<<<<<<<<<<<<Creating SBOM>>>>>>>>>>>>"
cd "$BASE_DIR"
docker run --rm -v "$PROJECT_DIR:/project" anchore/syft:latest /project -o json --source-version=1.0.0 | jq '.' > "$SBOM_PATH"
echo "<<<<<<<<<<<<Created SBOM>>>>>>>>>>>>"
